...
# API Routes
@app.route('/api/compliance-plan', methods=['POST'])
def generate_compliance_plan():
    try:
        ...
        if not waste or not location:
            return jsonify({"error": "Waste material and location are required"}), 400
...
def health_check():
    return jsonify({"status": "healthy", "service": "Waste Compliance API"})
