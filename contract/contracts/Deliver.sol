...
contract MarketWaste is ReentrancyGuard {
    ...
    // If an auto-payment fails, funds are stored here for manual claim.
    ...
    // Updated Event: Adds boolean to indicate if payout was instant (true) or queued for withdraw (false)
    ...
    function updateDeliveryStatus(uint256 _orderId, DeliveryStatus _status) public {
        ...
        // Prevent setting Delivered manually. Delivered is set by confirmations only.
    ...
    // --- INTERNAL PAYOUT LOGIC (HYBRID PUSH/PULL) ---
    function _tryCompleteOrder(uint256 _orderId) internal nonReentrant {
        ...
        if (order.agentConfirmed && order.buyerConfirmed) {
            ...
            if (order.paymentMethod == PaymentMethod.ETH) {
                ...
                // 3. Pay Seller (Remainder)
                sellerPayout = total - adminFee - agentFee;
                bool success = _safeTransfer(listing.seller, sellerPayout);

                // Emitting success status helps frontend know if they got money instantly or need to withdraw
    ...
    // Helper: Tries to Push funds, falls back to Pull (Withdrawal) on failure
    function _safeTransfer(address _to, uint256 _amount) internal returns (bool) {
        if (_amount == 0) return true;
        
        (bool sent, ) = _to.call{value: _amount}("");
        if (!sent) {
            // Push failed, enable Pull
    ...
    // --- WITHDRAWAL FUNCTION ---
    // Users call this if the automatic payout failed (or for refunds that used fallback)
    ...
    function refundOrder(uint256 _orderId) public nonReentrant {
        ...
        if (order.paymentMethod == PaymentMethod.ETH) {
            // Safe Refund: Try Push, fallback to Pull
